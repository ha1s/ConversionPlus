...
Процедура ЗарегистрироватьИзменения(Знач Узел, Знач ОбъектРегистрации)
	...
	Если ДоступнаРегистрацияСредствамиБСП Тогда
		МодульБСП = Вычислить("ОбменДаннымиСобытия");
		МодульБСП.ЗарегистрироватьИзмененияДанных(Узел, ОбъектРегистрации, Ложь);
	Иначе
...
Функция БСП_КонтрольВыгрузкиОбъекта(Узел, ОбъектРегистрации)
	...
	МодульОбменДаннымиПовтИсп = ОбщегоНазначенияОбщийМодуль("ОбменДаннымиПовтИсп");
	Если МодульОбменДаннымиПовтИсп <> Неопределено Тогда
		РежимВыгрузкиОбъекта = ОбменДаннымиПовтИсп.РежимВыгрузкиОбъекта(ОбъектРегистрации.Метаданные().ПолноеИмя(), Узел);
		Если РежимВыгрузкиОбъекта = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
			// Выгружаемые по ссылке можно всегда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
