...
Процедура ЗагрузитьЭлементРегистрации()
	...
	//{benony
	Если СтруктураРегистрации.Свойство("Ид") Тогда
		ПрочитанныйУИД = Новый УникальныйИдентификатор(СокрЛП(СтруктураРегистрации.Свойство["Ид"]));
		НайденнаяРегистрация = bnГитфлоу.НайтиЭлементСправочникаПоУИД(ПрочитанныйУИД, "Регистрации");
		Если НайденнаяРегистрация = Неопределено Тогда
			РегистрацияОбъект.УстановитьСсылкуНового(Справочники.Регистрации.ПолучитьСсылку(ПрочитанныйУИД));
		КонецЕсли;
	КонецЕсли;
	//}benony
